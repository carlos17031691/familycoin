Familycoin Core version 0.13.2 is now available from:

  <https://download.familycoin.org/familycoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/familycoin-project/familycoin/issues>
...
updates will be released anymore. Without security updates, using a familycoin
...
Familycoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Familycoin Core 0.13.2 also introduces new default policy limits on the length and
...
- Adding `dbcache=100` in familycoin.conf
...
familycoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/familycoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Familycoin Core code base has
...
- `familycoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `familycoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Familycoin scripting system that in combination with
...
Familycoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Familycoin Core.
...
The two phases are distinct in the Familycoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Familycoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Familycoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Familycoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of familycoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of familycoins being spent is exactly
...
- **Increased security for multisig:** Familycoin addresses (both P2PKH addresses
  ...
  believed to be provided by Familycoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Familycoin's current ledger (called the UTXO set),
  ...
  note that Familycoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Familycoin users to individually opt-in to almost any change in the Familycoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Familycoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Familycoin
client to Familycoin Core 0.13.2 to signal for segwit support. When a
...
Since Familycoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `familycoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `familycoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
