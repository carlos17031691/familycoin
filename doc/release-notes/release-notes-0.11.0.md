Familycoin Core version 0.11.0 is now available from:

  <https://familycoin.org/bin/familycoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer FAMILY/kB fee to be
...
For example, add the following to `familycoin.conf`:
...
related to the blockchain in the familycoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Familycoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Familycoin Core will 
...
For an example script see [familycoin-submittx](https://github.com/laanwj/familycoin-submittx).
...
- #5334 `cf87536` libfamilycoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new familycoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only FamilycoinAmountField
- #5753 `0f44672` Add familycoin logo to about screen
...
- #5809 `46bfbe7` Add familycoin-cli man page
...
- #5849 `41259ca` contrib/init/familycoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Familycoin-Qt.app to Familycoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for familycoin-tx)
