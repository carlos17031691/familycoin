 ...
 * between familycoind, and familycoin-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("familycoin-shutoff");
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", FAMILYCOIN_CONF_FILENAME), false, OptionsCategory::OPTIONS);
...
#ifndef WIN32
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", FAMILYCOIN_PID_FILENAME), false, OptionsCategory::OPTIONS);
#else
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/familycoin/familycoin>";
    const std::string URL_WEBSITE = "<https://familycoincore.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("familycoin-loadblk");
...
/** Sanity checks
 *  Ensure that Familycoin is running in a usable environment with all
...
{
    // Make sure only a single Familycoin process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", FAMILYCOIN_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

    // Warn about relative -datadir path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if familycoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if familycoin is started while in a temporary directory.\n",
