This directory contains the FamilycoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Familycoin UI.
...
ClientModel is used by the main application `familycoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/familycoin/familycoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a familycoin:... link from a browser).
...
* UI elements like FamilycoinAmountField, which inherit from QWidget.
* `familycoinstrings.cpp`: automatically generated
* `familycoinunits.(h/cpp)`: FAMILY / mFAMILY / etc handling
* `callback.h`
...
* don't change `local/familycoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "familycoin-qt" under "Run", which is where you can also add command line arguments)
