...
const char * const FAMILYCOIN_CONF_FILENAME = "familycoin.conf";
const char * const FAMILYCOIN_PID_FILENAME = "familycoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "familycoind -foo=bar
...
#else
    const char* pszModule = "familycoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Familycoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Familycoin
    // Mac: ~/Library/Application Support/Familycoin
    // Unix: ~/.familycoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Familycoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Familycoin";
#else
    // Unix
    return pathRet / ".familycoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", FAMILYCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", FAMILYCOIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Familycoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Familycoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Familycoin Core developers";
    }
