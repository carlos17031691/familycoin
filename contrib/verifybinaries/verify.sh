#!/usr/bin/env bash
# Copyright (c) 2016 The Familycoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   familycoincore.org and familycoin.org and compares them.
...
WORKINGDIR="/tmp/familycoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://familycoincore.org"
HOST2="https://familycoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="familycoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'familycoin-',
...
if [ $? -ne 0 ]; then
   echo "familycoin.org failed to provide signature file, but familycoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "familycoin.org and familycoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Familycoin Core binary release signing key installed?"
   fi
